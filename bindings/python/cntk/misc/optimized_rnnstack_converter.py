...
def _from_optimized_rnnstack(hipdnn_rnn):
    '''
    converts hipdnn optimized_rnnstack to non-hipdnn functions to run in non-CUDA environment
    
    Args:
        hipdnn_rnn: the optimized_rnnstack function that contains the parameters to be converted
    Returns:
        converted rnn function on GEMM based implementation that can be used on CPU
    '''

    if hipdnn_rnn.root_function.op_name != 'OptimizedRNNStack':
        raise ValueError('unexpected hipdnn_rnn.root_function.op_name value "%s"'%hipdnn_rnn.root_function.op_name)
    
    hipdnn_param = hipdnn_rnn.parameters[0]
    rnn_name = hipdnn_rnn.name
    input_var = hipdnn_rnn.inputs[0]
    
    hidden_size = hipdnn_rnn.root_function.attributes['hiddenSize']
    num_layers = hipdnn_rnn.root_function.attributes['numLayers']
    bidirectional = hipdnn_rnn.root_function.attributes['bidirectional']
    recurrent_op = hipdnn_rnn.root_function.attributes['recurrentOp']

    if recurrent_op not in ['lstm', 'rnnReLU', 'rnnTanh']:
        raise ValueError('unsupported recurrent_op value "%s"'%recurrent_op)
    #note that hipdnn GRU is different from standard GRU so no conversion unless creating a new type of GRU cell for CPU

    def _any_inferred(shape):
        return np.any([dim < 0 for dim in shape])
    
    if _any_inferred(hipdnn_param.shape) or _any_inferred(input_var.shape):
    ...
    nonhipdnn_func = rnn_lambda(input_var, '0')

    param = hipdnn_param.value.reshape(-1)
    offset = 0
    ...
    def _get_hipdnn_rnn_weight_splitter(in_dim, h_dim):
    ...
    def _get_hipdnn_rnn_bias_splitter(h_dim):
    ...
    for layer in range(num_layers):
        ...
        if bidirectional:
            fw_Wt, fw_Ht, bw_Wt, bw_Ht = np.split(layer_param, _get_hipdnn_rnn_weight_splitter(layer_input_size, hidden_size))
            fw_cell = nonhipdnn_func.find_by_name(rnn_name+'_fw'+layer_name, -1)
            bw_cell = nonhipdnn_func.find_by_name(rnn_name+'_bw'+layer_name, -1)
            ...
            Wt, Ht = np.split(layer_param, _get_hipdnn_rnn_weight_splitter(layer_input_size, hidden_size))
            cell = nonhipdnn_func.find_by_name(rnn_name+'_'+layer_name, -1)
        ...
        if layer != num_layers - 1:
            nonhipdnn_func = rnn_lambda(nonhipdnn_func.output, str(layer+1))
    ...
    for layer in range(num_layers):
        ...
        layer_name = str(layer)
        if bidirectional:
            fw_b1, fw_b2, bw_b1, bw_b2 = np.split(layer_param, _get_hipdnn_rnn_bias_splitter(hidden_size))
            fw_cell = nonhipdnn_func.find_by_name(rnn_name+'_fw'+layer_name, -1)
            bw_cell = nonhipdnn_func.find_by_name(rnn_name+'_bw'+layer_name, -1)
            fw_cell.b.value = _adjust_gate_order(fw_b1 + fw_b2).reshape(-1)
            bw_cell.b.value = _adjust_gate_order(bw_b1 + bw_b2).reshape(-1)
        else:
            b1, b2 = np.split(layer_param, _get_hipdnn_rnn_bias_splitter(hidden_size))
            cell = nonhipdnn_func.find_by_name(rnn_name+'_'+layer_name, -1)
            cell.b.value = _adjust_gate_order(b1 + b2).reshape(-1)
        offset += layer_size

    return nonhipdnn_func
    

def convert_optimized_rnnstack(hipdnn_model):
    '''
    Converts model that contains hipdnn optimized_rnnstack to use non-hipdnn functions, 
    so it can be used in non-CUDA environment

    Args:
        hipdnn_model: a model that contains optimized_rnnstacks
    Returns:
    ...
    class CuDNNOptimizedRNNConverter:
        def __init__(self, model):
            ...
            all_hipdnn_rnns = C.logging.graph.depth_first_search(model, self.filter, depth=-1)
            unique_params = set([hipdnn_rnn.parameters[0] for hipdnn_rnn in all_hipdnn_rnns])
            self.map_param_to_func = {p:None for p in unique_params}

        def converter(self, hipdnn_rnn):
            param = hipdnn_rnn.parameters[0]
            if self.map_param_to_func[param]:
                #shared parameter, clone
                converted = self.map_param_to_func[param][0].clone(C.CloneMethod.share, {self.map_param_to_func[param][1] : hipdnn_rnn.inputs[0], self.map_param_to_func[param][2] : C.placeholder()})
            else:
                #unique or first parameter, convert
                converted = _from_optimized_rnnstack(hipdnn_rnn)
                self.map_param_to_func[param] = (converted, hipdnn_rnn.inputs[0], hipdnn_rnn.output,)
            
            return converted

    optimizedRNNConverter = CuDNNOptimizedRNNConverter(hipdnn_model)
    return convert(hipdnn_model, optimizedRNNConverter.filter, optimizedRNNConverter.converter)
